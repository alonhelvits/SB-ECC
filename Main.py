"""
Score-Based Error Correction Codes (SB-ECC)
Training script for neural decoder.
"""
...
def train(model, device, train_loader, optimizer, epoch, LR):
    ...
    for batch_idx, (m, x, z, y, magnitude, syndrome) in enumerate(train_loader):
        loss, _ = model.loss(bin_to_sign(x), current_epoch=epoch)
        model.zero_grad()
        loss.backward()
        optimizer.step()
...
def test(model, device, test_loader_list, EbNo_range_test, solver_type='euler', min_FER=100, max_cum_count=1e8, min_cum_count=1e5):
    model.eval()
    ...
    with torch.no_grad():
        for ii, test_loader in enumerate(test_loader_list):
            ...
            # Initialize BER/FER lists with a dummy forward pass
            _, x_pred_list, _, _ = model.p_sample_loop(next(iter(test_loader))[3].to(device), solver_type=solver_type)
            ...
            while True:
                ...
                x_pred, x_pred_list, idx_conv, synd_all = model.p_sample_loop(y, solver_type=solver_type)
...
def main(args):
    ...
    # Create model
    model = SB_ECC(args, device=device, dropout=0).to(device)
    ...
    train_dataloader = DataLoader(
        FEC_Dataset(code, std_train, len=args.batch_size * 1000, zero_cw=False),
        batch_size=int(args.batch_size), shuffle=True, num_workers=args.workers)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (args.epochs > 1 and epoch % (args.epochs // 2) == 0) or epoch in [5, 15, 25, 50, 75, 100]:
            test(model, device, test_dataloader_list, EbNo_range_test, solver_type=args.solver,
                 min_FER=50, max_cum_count=1e6, min_cum_count=1e4)

    # Final evaluation
    logging.info('Loading Best Model')
    model = torch.load(os.path.join(args.path, 'best_model'), weights_only=False).to(device)
    logging.info('Final Evaluation')
    test(model, device, test_dataloader_list, EbNo_range_test, solver_type=args.solver, min_FER=500)


##################################################################

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SB-ECC Training')
    ...
    model_dir = os.path.join('SB_ECC_Results',
                             args.code_type + '__Code_n_' + str(args.code_n) + '_k_' + str(args.code_k) +
